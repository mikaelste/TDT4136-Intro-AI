...
class CSP:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # TODO: YOUR CODE HERE
        # Choose the first unassigned variable (you can implement more advanced strategies)
    ...
    def inference(self, assignment, queue):
        ...
        """
        while queue:
            (xi, xj) = queue.pop(0)  # Use pop(0) to pop from the front of the queue

            if self.revise(assignment, xi, xj):
                if len(assignment[xi]) == 0:
                    return False

                # Add all neighboring arcs of xi (except xj) back to the queue
    ...
    def revise(self, assignment, i, j):
        ...
        for i_value in assignment[i][:]:
            if not any((i_value, j_value) in self.constraints[i][j] for j_value in assignment[j]):
